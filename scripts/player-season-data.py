...
def batter_card(player, mobile=False):
    ...
    DATA = {
        ...
        'key': [['OBP', 'on-base pct (H+BB+HBP per PA)'], ['SLG', 'total bases per AB'], ['OPS', 'OBP + SLG'],
                ['ISO', 'isolated power (SLG − AVG)'], ['BABIP', 'batting avg on balls in play'],
                ['BB% / K%', 'walks / strikeouts per PA'], ['XBH · TB', 'extra-base hits · total bases'],
                ['SB%', 'stolen-base success (SB ÷ attempts)'], ['SF', 'sacrifice flies (not an AB)']],
...
def pitcher_card(player, mobile=False):
    ...
    DATA = {
        ...
        'key': [['FIP', 'fielding-independent pitching (HR, BB, K)'], ['BABIP', 'avg on balls in play'],
                ['K% / BB%', 'per batter faced'], ['WHIP', 'walks + hits per IP'],
                ['P/IP', 'pitches per inning'], ['BF', 'batters faced'], ['ERA', 'earned runs per 9 IP']],
